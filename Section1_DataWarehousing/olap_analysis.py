"""
DSA 2040 Practical Exam - OLAP Analysis and Visualization
Task 3: Execute OLAP queries and create visualizations
Author: IRANZI513
Date: August 14, 2025
"""
...
class OLAPAnalysis:
    """
    Class to execute OLAP queries and create visualizations
    """
    ...
    def drilldown_analysis(self):
        """
        Execute Drill-down OLAP operation: Sales details for UK by month
        """
        query = """
        SELECT 
            dc.Country,
        ...
        return self.execute_query(query, "DRILL-DOWN: Sales details for United Kingdom by month")
    
    def slice_analysis(self):
        """
        Execute Slice OLAP operation: Total sales for Electronics category
        """
        query = """
        SELECT 
            dp.ProductCategory,
        ...
        return self.execute_query(query, "SLICE: Total sales for Electronics category")
    ...
    def create_visualizations(self):
        ...
        if not monthly_df.empty:
            ...
            line = axes[1, 0].plot(monthly_df['Period'], monthly_df['MonthlyRevenue'], 
            ...
            n = max(1, len(monthly_df) // 10)
            xticks = axes[1, 0].get_xticks()
            axes[1, 0].set_xticks(xticks[::n])
        ...
        if not product_df.empty:
            ...
            axes[1, 1].set_yticklabels([f"{name[:20]}..." if len(name) > 20 else name 
...
- **Country**: {top_country.iloc[0]['Country']}
...
### Best Performing Category
- **Category**: {top_category.iloc[0]['ProductCategory']}
